...
In a recent test, the JMS processed and bulk inserted the 53 giga-byte Enron Microsoft Outlook PST dataset into RavenDB in a little over 30 minutes with the JMS, 2 Platform API instances, and RavenDB running on the same 16-core server. That comes out to over 100 GB/hour processing rate just based on input document size and not expanded dataset size.
