...
** This test processing rate was for the .NET 4.62 version of SDK, the new .NET 5 version is > 100% faster on average, all PST processing task on the .NET 5 version of OpenDiscoverPlatorm processed their PST dataset tasks between 90-100+GB/hr rates with sensitive item detection enabled (processing rates are hardware dependent - but in this case we used a single desktop PC with Intel i7 CPU and 16GB RAM).
